...
function draw() {
  ...
  player.show();
  player.move();
  // Show and move all enemies in the array
  for (let i = 0; i < enemies.length; i++) {
    ...
    for (let j = 0; j < bullets.length; j++) {
      if (bullets[j].collidesWith(enemies[i])) {
        ...
        bullets.splice(j, 1);
        enemies.splice(i, 1);
        // Decrement the index to account for the removed enemy
        i--;
        // Exit the inner loop since the bullet can only hit one enemy
        break;
      }
    }
  }
  ...
  // Show and move all bullets in the array
  ...
  if (enemies.length === 0) {
    ...
    // Convert the current time to Morse code and display it
    let now = new Date();
